                            ...
                            <tr class="booking-row" data-user="{{ strtolower($booking->user->name) }}" data-email="{{ strtolower($booking->user->email) }}" data-parking-lot="{{ strtolower($booking->parkingLot->name) }}" data-status="{{ $booking->status }}" data-parking-lot-id="{{ $booking->parking_lot_id }}" data-date="{{ $booking->start_time->format('Y-m-d') }}">
                                ...
                                <td class="p-2 text-sm leading-normal text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                    ...
                                    @elseif($booking->status === 'confirmed')
                                        <span class="px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white" style="background: linear-gradient(135deg, #3b82f6 0%, #60a5fa 100%);">
                                            Đã xác nhận
                                        </span>
                                    @elseif($booking->status === 'active')
                                        <span class="px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white" style="background: linear-gradient(135deg, #8b5cf6 0%, #a78bfa 100%);">
                                            Đang hoạt động
                                        </span>
                                ...
                                <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                    <div class="flex items-center justify-center flex-wrap gap-2">
                                        <a href="{{ route('admin.bookings.show', $booking) }}" class="btn-chip btn-blue" title="Xem chi tiết">
                                            <i class="fas fa-eye btn-icon"></i>
                                            <span class="hidden sm:inline">Xem</span>
                                        </a>
                                        <a href="{{ route('admin.bookings.edit', $booking) }}" class="btn-chip btn-emerald" title="Chỉnh sửa">
                                            <i class="fas fa-edit btn-icon"></i>
                                            <span class="hidden sm:inline">Sửa</span>
                                        </a>
                                        @if($booking->status !== 'completed')
                                        <form method="POST" action="{{ route('admin.bookings.destroy', $booking) }}" class="inline-flex" onsubmit="return confirm('Bạn có chắc chắn muốn xóa booking này?')">
                                            @csrf
                                            @method('DELETE')
