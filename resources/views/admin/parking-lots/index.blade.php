                            ...
                            <tr class="parking-lot-row" data-name="{{ strtolower($parkingLot->name) }}" data-address="{{ strtolower($parkingLot->address) }}" data-status="{{ $parkingLot->status }}">
                                ...
                                <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                    <div class="flex items-center justify-center flex-wrap gap-2">
                                        <a href="{{ route('admin.parking-lots.show', $parkingLot) }}" class="btn-chip btn-blue" title="Xem chi tiết">
                                            <i class="fas fa-eye btn-icon"></i>
                                            <span class="hidden sm:inline">Xem</span>
                                        </a>
                                        <a href="{{ route('admin.parking-lots.edit', $parkingLot) }}" class="btn-chip btn-emerald" title="Chỉnh sửa">
                                            <i class="fas fa-edit btn-icon"></i>
                                            <span class="hidden sm:inline">Sửa</span>
                                        </a>
                                        <form method="POST" action="{{ route('admin.parking-lots.destroy', $parkingLot) }}" class="inline-flex" onsubmit="return confirm('Bạn có chắc chắn muốn xóa bãi đỗ xe này?')">
                                            @csrf
                                            @method('DELETE')
